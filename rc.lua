...
tags = {
   names = { "term", "dev", "web", "im", "mail", "cal", "rss", 8, 9 },
   layout = { layouts[6], layouts[6], layouts[6], layouts[2], layouts[6], 
               layouts[6], layouts[6], layouts[6], layouts[6] }
}
for s = 1, screen.count() do
    -- Each screen has its own tag table.
    tags[s] = awful.tag(tags.names, s, tags.layout)
    awful.tag.setproperty(tags[s][4], "mwfact", 0.18)
end
-- }}}
...
cpugraph:set_gradient_angle(0):set_gradient_colors({
    beautiful.fg_end_widget, beautiful.fg_center_widget, beautiful.gf_widget
})
vicious.register(cpugraph, vicious.widgets.cpu, "$1")

-- weather widget
weatherwidget = widget({ type = "textbox" })
vicious.register(weatherwidget, vicious.widgets.weather, "${tempf}°F", 1201, "KALB")

-- Create a textclock widget
mytextclock = awful.widget.textclock({ align = "right" }, "%a, %b %d, %H:%M")

-- Create calendar for clock hover
calendar3.addCalendarToWidget(mytextclock)
...
awful.rules.rules = {
    ...
    --remove titlebar from MATLAB function hints 
    { rule = { name = "FunctionHints" },
      callback = awful.titlebar.remove },
    { rule = { class = "com-mathworks-util-PostVMInit", name = "Help" },
      properties = { floating = true } },
    { rule = { name = "Porous Material Models" },
      properties = { floating = true } },
    { rule = { name = "Impulse Analyzer" },
      properties = { floating = true } },
